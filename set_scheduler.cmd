...
>>%cmd.log% echo %time% ^> SCHTASKS /Create /RU SYSTEM /SC ONSTART /TN "%vsuh.task.name%" /TR %vsuh.task.cmd% /F
>>%cmd.log% 2>&1           SCHTASKS /Create /RU SYSTEM /SC ONSTART /TN "%vsuh.task.name%" /TR %vsuh.task.cmd% /F

echo err=%ERRORLEVEL%
tasklist /FI "IMAGENAME eq %vsuh.task.exe%*"|find /I "%vsuh.task.exe%" && ( 
	>>%cmd.log%  echo %time% Программа %vsuh.task.exe% уже выполняется
	) || (
